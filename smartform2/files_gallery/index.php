...
class Config
{
  ...
  public function __construct()
  {
    ...
    if (U::basename(__FILE__) === 'install.php')
      self::$config['allow_settings'] = true;
    ...
    if (self::get('allow_all'))
      foreach (['upload', 'delete', 'rename', 'new_folder', 'new_file', 'duplicate', 'text_edit', 'zip', 'unzip', 'move', 'copy', 'download', 'mass_download', 'mass_copy_links'] as $k)
        self::$config['allow_' . $k] = true;
    ...
    if (!self::$root)
      U::error('root dir <b>' . self::get('root') . '</b> does not exist');

    // storagepath can't be the same as root dir, because storage_path is excluded
    if (self::$storagepath === self::$root)
      U::error('storage_path can\'t be the same as root');
  ...
  private function storage()
  {
    ...
    if (self::get('storage_path') === false) {
      foreach (['cache', 'image_resize_cache', 'folder_preview_image'] as $key)
    ...
    if (!$path || !is_string($path))
      U::error('Invalid storage_path parameter');
    ...
    if (!self::$storagepath)
      U::error('storage_path does not exist and can\'t be created');
    ...
    if (!empty(self::$storageconfig))
      self::$config = array_replace(self::$default, self::$storageconfig, self::$localconfig);
    ...
    foreach (['config', 'cache/images', 'cache/folders', 'cache/menu'] as $dir)
      U::mkdir(self::$storagepath . '/' . $dir);

    // create or update config file if older than index.php
    if (!file_exists(self::$storageconfigpath) || filemtime(self::$storageconfigpath) < filemtime(__FILE__))
      self::save();
  }
...
class Login
{
  ...
  public function __construct()
  {
    ...
    if (session_status() === PHP_SESSION_NONE && !session_start())
      U::error('Failed to initiate PHP session_start()', 500);
  ...
  private function is_login_attempt()
  {
    ...
    if (U::get('action') === 'login' && empty($_POST))
  ...
  private function is_successful_login()
  {
    ...
    if (!U::post('fusername') || !U::post('fpassword') || !U::post('token') || !isset($_SESSION['token']))
      return false;

    // make sure $_SESSION token matches $_POST token
    if (!$this->equals($_SESSION['token'], U::post('token')))
      return false;
    ...
    if (!$this->user || !$this->passwords_match($this->user['password'], $_POST['fpassword']))
      return false;
  ...
  private function get_user($username)
  {
    ...
    foreach (glob(self::users_dir() . '/*', GLOB_ONLYDIR | GLOB_NOSORT) as $dir) {
      ...
      if ($lower_username === $this->lower($dirname))
        return $this->get_user_config($dirname);
    }
  }
  ...
  private function get_user_config($dirname)
  {
    ...
    if (empty($user['password']))
      return $this->error('User does not have valid password');
  ...
  private function passwords_match($stored, $posted)
  {
    if (password_get_info($stored)['algoName'] === 'unknown')
      return $this->equals($stored, $posted);
  ...
  private function action_request()
  {
    ...
    if (!U::get('action') || U::get('action') === 'tests')
      return false;
  ...
  private function get_form_alert()
  {
    ...
    if (isset($_POST['fusername']))
      return $this->alert('invalid login', 'danger');
...
class U
{
  ...
  public static function mkdir($path)
  {
    if (!file_exists($path) && !mkdir($path, 0777, true))
      U::error('Failed to create ' . $path, 500);
  }
  ...
  public static function uinclude($file)
  {
    ...
    if (!$src)
      return; // return if storagepath is non-public (not inside document root)
  ...
  public static function ini_value_to_bytes($directive)
  {
    ...
    if (function_exists('ini_parse_quantity'))
      return @ini_parse_quantity($val) ?: 0;
    if (!preg_match('/^(\d+)([G|M|K])?$/i', trim($val), $m))
      return 0;
    if (!isset($m[2]))
      return (int) $m[1];
  ...
  private static function exec_app_path($app_path)
  {
    ...
    foreach (['load_images', 'image_resize_cache'] as $key)
      if (!Config::get($key))
        return;
  ...
  public static function ffmpeg_path()
  {
    ...
    foreach (['video_thumbs', 'video_ffmpeg_path'] as $key)
      if (!Config::get($key))
        return;
  ...
  public static function imagemagick_path()
  {
    ...
    // below config options must be enabled for FFmpeg to apply
    foreach (['pdf_thumbs', 'imagemagick_path'] as $key)
      if (!Config::get($key))
        return;
  ...
  public static function readfile($path, $mime, $message = false, $cache = false, $clone = false)
  {
    if (!$path || !file_exists($path))
      return false;
    if ($clone && @copy($path, $clone))
      U::message('cloned to ' . U::basename($clone));
    U::header($message, $cache, $mime, filesize($path), 'inline', U::basename($path));
    if (!is_readable($path) || readfile($path) === false)
      U::error('Failed to read file ' . U::basename($path), 400);
    exit;
  }
  ...
  {
    if (isset(self::$image_resize_cache_direct))
      return self::$image_resize_cache_direct;
  ...
  {
    if (isset(self::$image_resize_dimensions_retina))
      return self::$image_resize_dimensions_retina;
  ...
  public static function html_header($title, $class)
  {
    ...
    <head>
      ...
      <link rel="apple-touch-icon"
        href="data:image/png;base64,">
      <meta name="mobile-web-app-capable" content="yes">
      <title><?php echo $title; ?></title>
      <link href="<?php echo U::assetspath(); ?>files.photo.gallery@<?php echo Config::$version ?>/css/files.css"
        rel="stylesheet">
      <?php // various custom includes
          U::uinclude('include/head.html');
          U::uinclude('css/custom.css');
          if (Login::$is_logged_in && !Login::$is_default_user)
            U::uinclude('users/' . Config::get('username') . '/css/custom.css');
          ?>
    </head>
    <?php
  }
  ...
  public static function header($message, $cache = null, $type = false, $length = 0, $disposition = false, $filename = '')
  {
    ...
    if ($message)
      array_unshift(self::$messages, $message);

    // append PHP response time to $messages
    if (isset($_SERVER['REQUEST_TIME_FLOAT']))
      self::$messages[] = round(microtime(true) - $_SERVER['REQUEST_TIME_FLOAT'], 3) . 's';

    // append memory usage to $messages
    if (function_exists('memory_get_peak_usage'))
      self::$messages[] = round(memory_get_peak_usage() / 1048576, 1) . 'M';

    // assign files-message header with all $messages
    header('files-response: ' . implode(' | ', self::$messages));

    // Wenn es sich um einen Bildtyp handelt, immer Cache-Control auf no-store setzen
    if ($type && strtok($type, '/') === 'image') {
  ...
  public static function save_config_file($dir, $data, $check_pass = false)
  {
    ...
    if (@preg_match_all("/^<\?php|return|password/", $data) < 3)
      return Json::error('Invalid user config');
    ...
    if (!@file_put_contents($test_file, $data))
      return Json::error('Failed to write user config');
    ...
    // validate root dir
    if (!empty($test['root']) && !is_dir($test['root']))
      return Json::error('Invalid root dir');

    // password must exist
    if ($check_pass && empty($test['password']))
      return Json::error('Config must contain a password');
    ...
    // save config.php
    if (!@file_put_contents("$dir/config.php", $data))
      return Json::error('Failed to write config file');
...
class Path
{
  ...
  private static function get_root_url_path()
  {
    ...
    if (is_string(Config::get('root_url_path')))
      return Config::get('root_url_path');
    ...
    if (self::is_within_docroot(Config::$root))
      return substr(Config::$root, strlen(Config::$document_root));
    ...
    if (!$root_parent_depth)
      return file_exists($trimmed_root) ? $trimmed_root : false;

    // attempt to assemble /root-relative path if root traverses up into parent dirs
    // must check PHP_SELF for comparison and PHP > 7
    if (!isset($_SERVER['PHP_SELF']) || version_compare(PHP_VERSION, '7.0.0') < 0)
      return false;
    ...
    if ($root_parent_depth > $php_self_depth)
      return false;
  ...
  public static function rooturlpath($rel)
  {
    ...
    if (!isset(self::$root_url_path))
      self::$root_url_path = self::get_root_url_path();

    // return false if if $root_url_path is false
    if (self::$root_url_path === FALSE)
      return false; //return self::urlpath($path);

    // return $root_url_path if relative path is empty (would be the root dir)
    if (!$rel)
      return self::$root_url_path;
  ...
  public static function urlpath($path)
  {
    ...
    if (!self::is_within_docroot($path))
      return false;

    // if item is within application dir, we can return relative path
    if (self::is_within_path($path, Config::$__dir__))
      return $path === Config::$__dir__ ? '.' : substr($path, strlen(Config::$__dir__) + 1);
  ...
  public static function valid_rootpath($relpath, $is_dir = false)
  {
    ...
    if ($realpath !== $rootpath && !self::is_within_path($realpath, Config::$root)) {
      if (strpos(($is_dir ? $relpath : dirname($relpath)), ':') !== false)
    ...
    if (!is_readable($realpath))
      return;        // not readable
    if ($is_dir && !is_dir($realpath))
      return;  // invalid dir
    if (!$is_dir && !is_file($realpath))
      return;// invalid file
    if (self::is_exclude($rootpath, $is_dir))
      return; // rootpath is excluded
  ...
  public static function is_exclude($path = false, $is_dir = true, $symlinked = false)
  {
    ...
    if (strpos('/' . self::relpath($path), '/_files') !== false)
      return true;
    ...
    if ($symlinked && !Config::get('allow_symlinks'))
      return true;

    // exclude Files Gallery storage_path (normally _files dir relative to PHP file)
    if (Config::$storagepath && self::is_within_path($path, Config::$storagepath))
      return true;
    ...
    if ($dirname !== Config::$root) {
      ...
      if (Config::get('dirs_include') && !preg_match(Config::get('dirs_include'), self::relpath($dirname)))
        return true;

      // exclude if `dirs_exclude` is assigned and $dirname matches dirs_exclude regex
      if (Config::get('dirs_exclude') && preg_match(Config::get('dirs_exclude'), self::relpath($dirname)))
        return true;
    }
    ...
    if (!$is_dir) {
      ...
      if ($filename === Config::$localconfigpath)
        return true;

      // exclude if `files_include` is assigned and $filename does not match files_include regex
      if (Config::get('files_include') && !preg_match(Config::get('files_include'), $filename))
        return true;

      // exclude if `files_exclude` is assigned and $filename matches files_exclude regex
      if (Config::get('files_exclude') && preg_match(Config::get('files_exclude'), $filename))
        return true;
    }
  }
}
...
class Json
{
  ...
  public static function cache($arr = [], $message = false, $cache = true)
  {
    ...
    if (empty($json))
      self::error(json_last_error() ? json_last_error_msg() : 'json_encode() error');
    if ($cache)
...
class X3
{
  ...
  {
    if (isset(self::$path))
      return self::$path; // serve previously resolved path
    ...
    foreach ([Config::$root, Config::get('root')] as $path) {
      ...
      if ($path && preg_match('/(.+)\/content/', $path, $match) && file_exists($match[1] . self::$inc))
        return self::$path = Path::realpath($match[1]);
    }
    // no match found
...
class Tests
{
  ...
  function __construct()
  {
    ...
    if (isset($_SERVER['SERVER_NAME']))
      $this->prop('<b>' . $_SERVER['SERVER_NAME'] . '</b>');
    $this->prop('<b>PHP ' . phpversion() . '</b>');
    if (isset($_SERVER['SERVER_SOFTWARE']))
      $this->prop('<b>' . $_SERVER['SERVER_SOFTWARE'] . '</b>');
    ...
    if (function_exists('extension_loaded'))
      foreach (['gd', 'exif', 'mbstring'] as $name)
        $this->prop($name, extension_loaded($name));
    ...
    foreach (['mime_content_type', 'finfo_file', 'iptcparse', 'exif_imagetype', 'session_start', 'ini_get', 'exec'] as $name)
      $this->prop($name . '()', function_exists($name));

    // check ffmpeg if exec() is available
    if (function_exists('exec'))
      $this->prop('ffmpeg', !!U::ffmpeg_path());

    // check imagemagick if exec() is available
    if (function_exists('exec'))
      $this->prop('imagemagick', !!U::imagemagick_path());
    ...
    if (function_exists('ini_get'))
      foreach (['memory_limit', 'file_uploads', 'upload_max_filesize', 'post_max_size', 'max_file_uploads'] as $name)
        $this->prop($name, 'neutral', @ini_get($name));

    // validate regex for exclude config options
    foreach (['files_include', 'files_exclude', 'dirs_include', 'dirs_exclude'] as $key)
      if (Config::get($key) && @preg_match(Config::get($key), '') === false)
        $this->prop("Invalid <strong>$key</strong> regex", false);
  ...
  private function check_path($path, $name)
  {
    ...
    if (!file_exists($path))
      return $this->prop("$name does not exist", false);
    if (!is_writable($path))
      return $this->prop($name . ' is not writeable ' . substr(sprintf('%o', fileperms($path)), -4) . ' [owner ' . fileowner($path) . ']', false);
    $this->prop($name, true);
  }
  ...
  private function showconfig()
  {
    ...
    foreach (['root', 'storage_path', 'start_path', 'username', 'password', 'license_key', 'allow_tasks', 'index_cache', 'files_include', 'files_exclude', 'dirs_include', 'dirs_exclude'] as $prop)
      if ($arr[$prop])
        $arr[$prop] = '***';
...
class FileResponse
{
  ...
  public function __construct($path, $resize = false, $clone = false)
  {
    ...
    if (!$path)
      U::error('Invalid file request', 404);
    ...
    if (in_array($this->resize, ['video', 'pdf']))
      return $this->get_exec_preview($this->resize);

    // get resized image preview (convert resize parameter to number, else it will return 0, not allowed)
    if ($this->resize)
      return $this->get_image_preview();
  ...
  private function get_exec_preview($type)
  {
    ...
    if (!Config::get('image_resize_cache'))
      U::error("image_resize_cache must be enabled to create and store $type previews", 400);

    // requirements / only check $mime if $mime detected
    if ($this->mime && strpos($this->mime, $this->resize) === false)
      U::error("Unsupported $type type $this->mime", 415);
    ...
    if ($cache)
      U::readfile($cache, 'image/jpeg', "$type preview from cache", true, $this->clone);
    ...
    if ($result_code)
      U::error("Error generating $type preview image (\$result_code $result_code)", 500);

    // if for some reason, the created $cache file does not exist
    if (!file_exists($cache))
      U::error('Cache file ' . U::basename($cache) . ' does not exist', 404);
    ...
    if (!filesize($cache) && imagejpeg(imagecreate(1, 1), $cache))
      U::readfile($cache, 'image/jpeg', '1px placeholder image created and cached', true, $this->clone);
  ...
  {
    if (empty($this->resize) || !is_numeric($this->resize))
      return false;
    if ($this->resize === Config::get('image_resize_dimensions'))
      return true;
    if ($this->resize === Config::get('image_resize_dimensions_retina'))
      return true;
  ...
  private function get_image_preview()
  {
    ...
    if ($this->mime && strtok($this->mime, '/') !== 'image')
      U::error('Unsupported image type ' . $this->mime, 415);
    ...
    foreach (['load_images', 'image_resize_enabled'] as $key)
      if (!Config::get($key))
        U::error("Config $key disabled", 400);

    // check if requested resize value is allowed
    if (!$this->resize_allowed())
      U::error("Resize parameter $this->resize is not allowed", 400);
  ...
  private function get_file_proxied()
  {
    ...
    if (!Config::get('load_files_proxy_php') && Path::is_within_docroot($this->path))
      U::error('File can\'t be proxied', 400);
...
class ResizeImage
{
  ...
  public function __construct($path, $resize, $clone = false)
  {
    ...
    if ($cache_path)
      U::readfile($cache_path, 'image/jpeg', 'Resized image from cache', true, $clone);
    ...
    if (empty($imagesize) || !is_array($imagesize))
      U::error('Failed getimagesize()', 500); // die!
    ...
    if (!in_array($ext, $this->get_resize_types($types))) {
      ...
      if ($filesize > Config::get('load_images_max_filesize'))
        U::error("Image $ext is not in `image_resize_types`, and can't serve original because filesize $filesize exceeds `load_images_max_filesize` " . Config::get('load_images_max_filesize') . ' bytes', 400);

      // attempt to serve original or error
      if (!U::readfile($this->path, $mime, 'Original image served because image is not within image_resize_types', true, $clone))
        U::error('File does not exist', 404);
    }

    // exit if image pixels (dimensions) exceeds 'image_resize_max_pixels' => 60000000 (default)
    if ($max_pixels && $this->pixels > $max_pixels)
      U::error("Image pixels $this->pixels ($width x $height) exceeds `image_resize_max_pixels` $max_pixels", 400);

    // serve original if resize ratio < min_ratio, but only if filesize <= load_images_max_filesize
    if ($ratio < max($min_ratio, 1) && $filesize <= Config::get('load_images_max_filesize') && !U::readfile($this->path, $mime, "Original image served, because resize ratio $ratio < min_ratio $min_ratio", true, $clone))
      U::error('File does not exist', 404);
    ...
    if (!function_exists($imagecreatefrom))
      U::error("Function $imagecreatefrom() does not exist", 500);
    ...
    if (!call_user_func($function, $this->dst_image, $src_image, 0, 0, 0, 0, $this->rwidth, $this->rheight, $width, $height))
      U::error("Function $function() failed", 500);
    ...
    if (!$cache_path)
      U::header('Resized image served', true, 'image/jpeg');

    // create jpg image in cache path or output directly if !cache
    if (!imagejpeg($this->dst_image, $cache_path, $quality))
      U::error('PHP imagejpeg() failed', 500);

    // destroy dst_image resource to free up memory
    imagedestroy($this->dst_image);

    // cache readfile
    if ($cache_path && !U::readfile($cache_path, 'image/jpeg', 'Resized image served', true, $clone))
      U::error('Cache file does not exist', 404);
  ...
  private function set_memory_limit($memory_limit)
  {
    ...
    if ($required > $new)
      U::error("Resizing this image requires >= {$required}M. Your PHP memory_limit is {$new}M", 400);
    // assign $new memory from config image_resize_memory_limit if > $current (default memory_limit)
    if ($new > $current && @ini_set('memory_limit', $new . 'M'))
      U::message("{$current}M => {$new}M (min {$required}M)");
  }
  ...
  private function exif_orientation()
  {
    ...
    if (!array_key_exists($orientation, $orientation_to_rotation))
      return;
  ...
  {
    if (!is_array(self::$fill_color) || count(self::$fill_color) !== 3 || !in_array($ext, ['png', 'gif', 'webp', 'avif']))
      return;
    $color = call_user_func_array('imagecolorallocate', array_merge([$this->dst_image], self::$fill_color));
    if (imagefill($this->dst_image, 0, 0, $color))
      U::message('Fill rgb(' . join(', ', self::$fill_color) . ')');
  }
}
...
class Dirs
{
  ...
  private function check_cache()
  {
    ...
    if (!$hash || !preg_match('/^.{6}\..{6}\..\d+$/', $hash))
      Json::error('Invalid menu cache hash');
    ...
    if (!$this->menu_cache_is_valid($json))
      return;
    ...
    if (U::post('localstorage'))
      exit(json_encode(['localstorage' => true]));
  ...
  {
    if (!Config::get('menu_cache_validate'))
      return true; // don't validate deep levels beyond 2
    ...
    foreach ($arr as $val) {
      if (strpos($val['path'], '/') !== false && $val['mtime'] !== @filemtime(Path::rootpath($val['path'])))
        return;
    }
  ...
  private function get_dirs($path, $depth = 0)
  {
    ...
    if ($depth || $this->load_files) {
      ...
      if (Config::get('menu_max_depth') && $depth >= Config::get('menu_max_depth'))
        return;

      // exit if item is symlink and don't follow symlinks (don't get subdirs)
      if ($data['is_link'] && !Config::get('menu_recursive_symlinks'))
        return;// $arr;
    }
    ...
    if (!empty($subdirs))
      foreach ($this->sort($subdirs) as $subdir)
        $this->get_dirs($subdir, $depth + 1);
  }
...
class Dir
{
  ...
  public function json()
  {
    ...
    if (U::readfile($this->cache_path, 'application/json', 'JSON served from cache'))
      return;
  ...
  public function get()
  {
    ...
    if ($this->cache_is_valid())
      return json_decode(file_get_contents($this->cache_path), true);
  ...
  public function load($files = false)
  {
    ...
    if ($files) {
      ...
      if (!isset($this->memory_limit_mb))
        $this->memory_limit_mb = U::get_memory_limit_mb();
      $this->get_files();
    }
  ...
  private function set_json_cache_url()
  {
    ...
    if (Login::$is_logged_in || !$this->cache_is_valid() || !Path::is_within_docroot(Config::$storagepath))
      return;
...
class File
{
  ...
  public function __construct($dir, $filename)
  {
    ...
    if (Path::is_exclude($path, $is_dir, $symlinked))
      return;

    // count file into parent dir
    if (!$is_dir)
      $this->dir->data['files_count']++;
  ...
  private function mime()
  {
    ...
    if (!$this->file['ext'] || $this->file['ext'] === 'ts' || Config::get('get_mime_type'))
      return U::mime($this->realpath);
  ...
  private function is_image()
  {
    if ($this->file['is_dir'])
      return;
    if ($this->file['mime'])
      return strpos($this->file['mime'], 'image/') === 0;
  ...
  private function getimagesize()
  {
    ...
    // exit
    if (!$this->file['is_readable'] || $this->file['ext'] === 'svg')
      return;
    ...
    foreach (['width', 'height', 'type', 'bits' => 'bits', 'channels' => 'channels', 'mime' => 'mime'] as $key => $name)
      if (isset($imagesize[$key]))
        $this->image[$name] = $imagesize[$key];
  ...
  {
    if (!isset($this->image['exif']['Orientation']) || !in_array($this->image['exif']['Orientation'], [5, 6, 7, 8]))
      return;
  ...
  private function image_panorama_sizes()
  {
    ...
    if (!$this->file['url_path'] || !$this->image['width'] || $this->image['width'] <= 2048 || $this->image['width'] / $this->image['height'] !== 2)
      return;
    $resized = [];
    foreach ([2048, 4096, 8192] as $width) { // look for sizes 2048, 4096 and 8192
      if ($width >= $this->image['width'])
        break; // break loop if resized image width >= original already
      if (file_exists($this->dir->realpath . '/_files_' . $width . '_' . $this->file['basename']))
        $resized[] = $width;
    }
    if (!empty($resized))
      $this->file['panorama_resized'] = array_reverse($resized);
  }

  // get image resize cache for direct access by javascript if config image_resize_cache_direct
  private function get_image_resize_cache()
  {
    if (!U::image_resize_cache_direct())
      return;
    foreach ([Config::get('image_resize_dimensions'), U::image_resize_dimensions_retina()] as $resize) {
      ...
      if (file_exists($cache_file))
        $this->image["resize$resize"] = Path::urlpath($cache_file);
    }
  }
  ...
  {
    if (!$this->file['is_readable'] || $this->file['ext'] !== 'url')
      return;
    $lines = @file($this->realpath);
    if (empty($lines) || !is_array($lines))
      return;
    foreach ($lines as $str) {
      if (!preg_match('/^url\s*=\s*([\S\s]+)/i', trim($str), $matches) || !isset($matches[1]))
        continue;
    ...
    foreach (['GPSLatitude', 'GPSLongitude'] as $key) {
      ...
      for ($i = 0; $i < 3; $i++) {
        ...
        if (count($part) == 1) {
          ...
          if (empty($part[1]))
            return false; // invalid GPS, $part[1] can't be 0
...
class Filemanager
{
  ...
  public static function json($res, $err)
  {
    ...
    if (!isset($arr['success']) || empty($arr['success']))
  ...
  public static function delete($path)
  {
    ...
    if (is_dir($path) && !is_link($path)/* && is_writable($path)*/)
      foreach (self::iterator($path, RecursiveIteratorIterator::CHILD_FIRST) as $item)
        self::$success += self::delete_file_or_folder($item->getPathname());
  ...
  private static function copy_file_or_folder($from, $to)
  {
    ...
    if (Path::is_within_path($to, $from))
      return false; // don't allow copying files or dirs into self or same location
    ...
    if (is_link($from)) {
      if (!function_exists('symlink'))
        return false; // can't proceed if symlink() doesn't work
    ...
    if (is_dir($from))
      return is_dir($to) || @mkdir($to, 0777, true); // is_dir already or make new dir
    if (!is_readable($from))
      return false; // can't read file source / might be recursive file
    if (file_exists($to) && filemtime($to) >= filemtime($from))
      return false; // file already exists and is newer than source
    if (!@copy($from, $to))
      return false; // attempt to copy from to / overwrite existing older files in $to location
  ...
  {
    if (!self::copy_file_or_folder($from, $to))
      return false; // only continue on success
    ...
    if (is_dir($from) && !is_link($from)) {
      ...
      foreach ($iterator as $descendant)
        self::$success += self::copy_file_or_folder($descendant, $to . '/' . $iterator->getSubPathName());
    }
    return true;
  }
  ...
  private static function move($from, $to)
  {
    ...
    if (Path::is_within_path($to, $from))
      return false; // don't allow moving files or dirs into self or same location
    if (file_exists($to) && filemtime($to) >= filemtime($from))
      return false; // $to already exists and is newer than $from
  ...
  public static function items($action, $paths, $dir = false)
  {
    ...
    foreach ($paths as $path)
      self::$success += self::$action($path, ($dir ? $dir . '/' . U::basename($path) : false));
  ...
  public static function get_downloadables($paths)
  {
    ...
    foreach ($paths as $dir) {
      ...
      if (Path::is_exclude($dir, true))
        continue; // shouldn't be necessary when forwarded from frontend, but just in case
      foreach (self::iterator($dir) as $item) { // loop dirs get all descendants
        ...
        if (!is_readable($path) || is_dir($path) || Path::is_exclude($path, false)/* || is_link($path)*/)
          continue;
...
class Zipper extends Filemanager
{
  ...
  public function __construct($is_json = false)
  {
    ...
    if (!class_exists('ZipArchive'))
      U::error('Missing PHP ZipArchive class', 500, $this->is_json);
  ...
  private function open($dest, $flags = null)
  {
    ...
    //  die if error
    if ($res !== true)
      U::error($res && isset($zip_open_errors[$res]) ? $zip_open_errors[$res] : 'Unknown zip error', 500, $this->is_json);
  }
  ...
  public function extract($zip_file, $dir = false)
  {
    ...
    // check valid zip
    if (!is_file($zip_file) || U::extension($zip_file, true) !== 'zip' || empty(filesize($zip_file)))
      U::error('Invalid zip file', 400, $this->is_json);
    ...
    if (!is_writable($dir))
      U::error('Target dir is not writeable', 403, $this->is_json);
  ...
  public function create($paths, $zip_file = false)
  {
    ...
    if (!$zip_file)
      $zip_file = self::get_unique_filename($root . (count($paths) > 1 ? 'archive.zip' : U::basename($first_path) . (U::extension($first_path, true) !== 'zip' ? '.zip' : '')));
    ...
    foreach ($paths as $path) {
      ...
      if ($this->add_file_or_dir($path, $root) && is_dir($path))
        foreach (self::iterator($path) as $file)
          $this->add_file_or_dir($file, $root);
    }
  ...
  private function add_file_or_dir($path, $root)
  {
    ...
    if (Path::is_exclude($path, is_dir($path)) || !is_readable($path))
      return false; // file excluded, continue
...
class Document
{
  ...
  private $index_html = false;          // timestamp when index.html index_cache
  ...
  public function __construct()
  {
    ...
    if ($this->absolute_start_path && Path::realpath($this->absolute_start_path) !== Config::$root)
      $this->dirs[$this->start_path] = (new Dir($this->absolute_start_path))->get();
  ...
  private function get_start_path()
  {
    ...
    if ($query_path) {
      ...
      if (!$this->absolute_start_path || !Path::is_within_path($this->absolute_start_path, Config::$root) || Path::is_exclude($this->absolute_start_path))
        U::error('Invalid start_path ' . $start_path);
  ...
  {
    if ($this->index_html)
      return; // exit if we are generating index.html index_cache, as query will be manageg by javascript
    if (empty($_SERVER['QUERY_STRING']))
      return; // exit if !QUERY_STRING
    $path = explode('&', $_SERVER['QUERY_STRING'])[0]; // get first parameter in QUERY_STRING for path
    if (!$path || strpos($path, '=') !== false)
      return; // make sure path exists and is not assigned parameter=value
  ...
  private function get_menu_cache_file()
  {
    ...
    if (Config::get('menu_cache_validate') || !Config::get('cache') || !Path::is_within_docroot(Config::$storagepath))
      return;
    ...
    if ($url_path)
      $this->menu_cache_file = $url_path . '?' . filemtime($path);
  }
  ...
  private function start_index_cache()
  {
    ...
    if (Config::get('index_cache') !== TRUE && U::get('create_index_cache') !== Config::get('index_cache'))
      U::error('Incorrect passphrase for index_cache');
    // index_cache does not work if login is required
    if (Login::$has_public_login)
      U::error('Config index_cache can\'t be used when login is required');
    // index_cache can only be created from non-logged in user (must create a public accessible html page)
    if (Login::$is_logged_in)
      U::error("You must <a href=\"?logout=1&create_index_cache=$create_index_cache\">logout</a> to generate index_cache");
  ...
  private function HTML()
  {
    ...
    // start index.html index_cache if enabled and assigned by url parameter ?create_index_cache=passphrase
    if (Config::get('index_cache') && U::get('create_index_cache'))
      $this->start_index_cache();
    ...
    <body class="body-loading" <?php if (Login::$is_logged_in)
      echo ' data-username="' . htmlspecialchars(Config::get('username')) . '"'; ?>>
      <main id="main">
        <nav id="topbar" class="topbar-sticky">
          <div id="topbar-top">
            <div id="search-container"><input id="search" class="input" type="search" placeholder="search" size="1"
                spellcheck="false" autocomplete="off" autocorrect="off" autocapitalize="off" disabled></div>
      ...
      // Globale Variablen in dieser Methode verfügbar machen
      global $gadget, $id;

      U::uinclude('js/functions.js');
      // load _files/js/custom.js if the file exists
  
      // load _files/js/custom.js if exists
      if ($gadget == 'photo') {
        U::uinclude("js/custom_photo.js?gadget=$gadget&id=$id");
      } else
...
// if(Login::$is_logged_in && !Login::$is_default_user) U::uinclude('users/' . Config::get('username') . '/js/custom.js');
      ...
      foreach (array_filter(['toastify-js@1.12.0/src/toastify.min.js', 'sweetalert2@11.12.3/dist/sweetalert2.min.js', 'animejs@3.2.2/lib/anime.min.js', 'yall-js@3.2.0/dist/yall.min.js', 'filesize@9.0.11/lib/filesize.min.js', 'screenfull@5.2.0/dist/screenfull.min.js', 'dayjs@1.11.12/dayjs.min.js', 'dayjs@1.11.12/plugin/localizedFormat.js', 'dayjs@1.11.12/plugin/relativeTime.js', (in_array(Config::get('download_dir'), ['zip', 'files']) ? 'js-file-downloader@1.1.25/dist/js-file-downloader.min.js' : false),
        ...
        echo '<script src="' . U::assetspath() . $key . '"></script>' . PHP_EOL;
      ?>
    </body>
  ...
  private function lang_custom()
  {
    ...
    foreach ($files as $path) { // loop language files
      ...
      if (!empty($data))
        $langs[strtok(U::basename($path), '.')] = $data; // assign language as array
    }
  ...
  {
    if (!Config::get('allow_upload') || !Config::$storagepath || !Path::is_within_docroot(Config::$storagepath))
      return false;
  ...
  {
    if (!Config::get('allow_upload'))
      return 0; // just return 0 if upload is disabled
...
if (U::get('action')) {
  ...
  if (!in_array($action, ['ping', 'settings', 'login', 'files', 'dirs', 'load_text_file', 'check_updates', 'do_update', 'save_license', 'delete', 'text_edit', 'unzip', 'rename', 'new_file', 'new_folder', 'zip', 'copy', 'move', 'duplicate', 'get_downloadables', 'upload', 'download_dir_zip', 'preview', 'file', 'download', 'tasks', 'tests']))
  ...
  if ($request->is_post === in_array($action, ['download_dir_zip', 'preview', 'file', 'download', 'tasks', 'tests']))
  ...
  if (isset(Config::$config['allow_' . $action]) && !Config::$config['allow_' . $action])
    $request->error("$action not allowed");

  // block all write actions in demo mode (that's what demo_mode option is for)
  if (Config::get('demo_mode') && in_array($action, ['upload', 'delete', 'rename', 'new_folder', 'new_file', 'duplicate', 'text_edit', 'zip', 'unzip', 'move', 'copy']))
    $request->error("$action not allowed in demo mode");

  // block all download actions [get_downloadables, download_dir_zip, download] if !allow_download
  if (!Config::get('allow_download') && strpos($action, 'download') !== false)
    $request->error('Download not allowed');

  // block all mass download actions [get_downloadables, download_dir_zip] if !allow_mass_download
  if (!Config::get('allow_mass_download') && in_array($action, ['get_downloadables', 'download_dir_zip']))
    $request->error('Mass download not allowed');
  ...
  if ($dir !== false) { // explicitly check !== false because $dir could be valid '' empty (root)
    ...
    if (!$dir)
      $request->error('Invalid dir path');
    // some actions require $dir to be writeable
    if (in_array($action, ['copy', 'move', 'unzip', 'upload']) && !is_writable($dir))
      $request->error('Dir is not writeable');
  ...
  if ($file) {
    ...
    if (!$file)
      $request->error('Invalid file path');
  ...
  if (isset($request->params['items'])) {
    ...
    if (empty($items) || !is_array($items))
      $request->error('Invalid items parameter');
    ...
    // no valid item paths
    if (empty($paths))
      $request->error('Invalid item paths');
    ...
    if ($first_path === Config::$root && !in_array($action, ['new_file', 'new_folder']))
      $request->error("Can't $action root directory");
    ...
    if ($name !== false) {
      ...
      if (!Filemanager::name_is_allowed($name))
        $request->error(trim("Invalid name $name"));
      ...
      if (!is_dir($parent_dir))
        $request->error('Not a directory'); // parent path must be dir
      if (!is_writable($parent_dir))
        $request->error('Dir is not writeable'); // dir must be writeable

      // assign $new_path from $name
      $new_path = $parent_dir . '/' . $name;
      if (file_exists($new_path))
        $request->error((is_dir($new_path) ? 'Dir' : 'File') . ' already exists');
    }
  ...
  if ($action === 'files') {
    ...
    if (@readfile($file) === false)
      U::error('failed to read file', 500);
    ...
    if (!$version || !Config::get('allow_check_updates') || version_compare($version, Config::$version) <= 0 || !is_writable(__FILE__))
      $request->error('Error');
    $get = @file_get_contents('https://cdn.jsdelivr.net/npm/files.photo.gallery@' . $version . '/index.php');
    if (empty($get) || strpos($get, '<?php') !== 0 || !@file_put_contents(__FILE__, $get))
    ...
    if (!isset($request->params['text']) || !is_string($request->params['text']))
      $request->error('Invalid text parameter');
    if (!is_writeable($first_path))
      $request->error('File is not writeable');
    if (!is_file($first_path))
      $request->error('Not a file');
    if (@file_put_contents($first_path, $request->params['text']) === false)
      $request->error('failed to write to file', 500);
    ...
    if (!$new_path)
      $request->error('Missing name parameter');
    ...
    if (empty($valid_copy_move_paths))
      $request->error("can't $action into self");
    ...
    if ($new_path)
      Filemanager::json(Filemanager::copy($first_path, $new_path), 'duplicate failed');
    ...
    // invalid $upload['size']
    if (!isset($upload['size']) || empty($upload['size']))
      $request->error('Invalid file size');

    // $upload['size'] must not exceed $config['upload_max_filesize']
    if (Config::get('upload_max_filesize') && $upload['size'] > Config::get('upload_max_filesize'))
      $request->error('File size [' . $upload['size'] . '] exceeds upload_max_filesize option [' . Config::get('upload_max_filesize') . ']');
    ...
    if (strpos($filename, '/') !== false || strpos($filename, '\\') !== false)
      $request->error('Illegal \slash/ in filename ' . $filename);
    ...
    if (!empty($allowed_file_types)) {
      ...
      // invalid file type
      if (!$is_valid)
        $request->error("Invalid file type $filename");

      // for additional security, check if uploaded image is an actual image with exif_imagetype() function
      if (function_exists('exif_imagetype') && in_array($ext, ['.gif', '.jpeg', '.jpg', '.png', '.swf', '.psd', '.bmp', '.tif', '.tiff', '.webp', '.avif']) && !@exif_imagetype($upload['tmp_name']))
        $request->error("Invalid image type $filename");
    }
    ...
    if (Config::get('upload_exists') === 'fail' && file_exists($move_path))
      $request->error("$filename already exists");

    // increment file name if file name already exists
    if (Config::get('upload_exists') === 'increment')
      $move_path = Filemanager::get_unique_filename($move_path);
    ...
    if (Config::get('download_dir') !== 'zip')
      $request->error('download_dir zip disabled');
    ...
    if (!Config::$storagepath || Config::get('download_dir_cache') === 'dir') {
      if (!is_writable($dir))
        $request->error('Dir is not writeable', 500);
    ...
    if (!$cached && !(new Zipper())->create([$dir], $zip_file))
      $request->error('Failed to create ZIP file', 500);

    // ignore user abort so we can delete file also on download cancel
    if (!Config::get('download_dir_cache'))
    ...
    foreach (['folder_preview_image', 'load_images', 'image_resize_enabled', 'image_resize_cache'] as $key)
      if (!Config::get($key))
        $request->error("Config option $key disabled", 403);
    ...
    if (file_exists($cache)) {
      ...
      if (filemtime($cache) >= filemtime($dir))
        U::readfile($cache, 'image/jpeg', 'Preview image from cache', true);
    ...
    if (!empty($files)) {
      ...
      foreach ($files as $file) {
        ...
        if (!$match)
          continue; // skip if extension not supported

        // skip if is_exclude or !readable
        if (Path::is_exclude($file, false) || !is_readable($file))
          continue;
    ...
    if (!U::uinclude('plugins/files.tasks.php'))
      $request->error('Can\'t find tasks plugin', 404);
    ...
    if (!Config::$storagepath)
      return Json::error('Storage path does not exist');
    ...
    if ($request->param('load')) {
      ...
      if ($users_dir)
        foreach (glob("$users_dir/*", GLOB_ONLYDIR) as $dir) {
    ...
    // create local $short vars from config 'image_resize_*' options, because it's much easier and more readable
    foreach (['username', 'new_name', 'is_new', 'is_rename', 'is_default', 'data', 'remove'] as $k)
      $$k = $request->param($k);

    // if is default _files/config/config.php, save and return with doing further checks
    if ($is_default)
      return Json::jexit(['success' => true, 'data' => U::save_config_file(Config::$storagepath . '/config', $data)]);

    // $username must be set and not equal default
    if (!$username || strtolower($username) === 'default')
      return Json::error('Invalid username' . ($username ? " '$username'" : ''));

    // if new_name (new user or renamed user) check if name is allowed (we don't need to check existing usernames)
    if ($new_name && !Filemanager::name_is_allowed($new_name))
    ...
    if (!$is_new && !file_exists("$user_dir/config.php"))
      return Json::error('Username does not exist');

    // REMOVE / Remove dir and return
    if ($remove)
      return Json::jexit(['success' => !!Filemanager::delete($user_dir)]);

    // if new user or rename, make sure dir doesn't already exist
    if ($new_name && file_exists($user_dir_new))
      return Json::error('Username already exists');

    // create user dir if is new user
    if ($is_new && !@mkdir($user_dir_new, 0777, true))
      return Json::error('Failed to create user dir');

    // validate and save config.php file
    $data = U::save_config_file($user_dir, $data, true);

    // rename user dir
    if ($is_rename && !@rename($user_dir, $user_dir_new))
      return Json::error('Failed to rename user');
