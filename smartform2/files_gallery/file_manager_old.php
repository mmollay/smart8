...
class config {
	...
	public static $default = array (
			...
			// paths
			'root' => '','start_path' => false,

			// login
			'username' => '','password' => '',

			// images
			'load_images' => true,'load_files_proxy_php' => false,'load_images_max_filesize' => 1000000,'image_resize_enabled' => true,'image_resize_cache' => true,'image_resize_dimensions' => 320,'image_resize_dimensions_retina' => 480,'image_resize_dimensions_allowed' => '','image_resize_types' => 'jpeg, png, gif, webp, bmp, avif','image_resize_quality' => 85,'image_resize_function' => 'imagecopyresampled','image_resize_sharpen' => true,'image_resize_memory_limit' => 128,'image_resize_max_pixels' => 30000000,'image_resize_min_ratio' => 1.5,'image_resize_cache_direct' => false,'folder_preview_image' => true,'folder_preview_default' => '_filespreview.jpg',

			// menu
			'menu_enabled' => true,'menu_show' => true,'menu_max_depth' => 5,'menu_sort' => 'name_asc','menu_cache_validate' => true,'menu_load_all' => false,'menu_recursive_symlinks' => true,

			// files layout
			'layout' => 'rows','sort' => 'name_asc','sort_dirs_first' => true,'sort_function' => 'locale',

			// cache
			'cache' => true,'cache_key' => 0,'storage_path' => '_files',

			// exclude files directories regex
			'files_exclude' => '','dirs_exclude' => '','allow_symlinks' => true,

			// various
			'title' => '%name% [%count%]','history' => true,'transitions' => true,'click' => 'popup','click_window' => '','click_window_popup' => true,'code_max_load' => 100000,'topbar_sticky' => 'scroll','check_updates' => false,'allow_tasks' => false,'get_mime_type' => false,'context_menu' => true,'prevent_right_click' => false,'license_key' => '','filter_live' => true,'filter_props' => 'name, filetype, mime, features, title','download_dir' => 'browser','download_dir_cache' => 'dir','assets' => '',

			// filemanager options
			'allow_upload' => false,'allow_delete' => false,'allow_rename' => false,'allow_new_folder' => false,'allow_new_file' => false,'allow_duplicate' => false,'allow_text_edit' => false,'demo_mode' => false,

			// uploader options
			'upload_allowed_file_types' => '','upload_max_filesize' => 0,'upload_exists' => 'increment',

			// popup options
			'popup_video' => true,

			// video
			'video_thumbs' => true,'video_ffmpeg_path' => 'ffmpeg',

			// language
			'lang_default' => 'en','lang_auto' => true );
...
<title><?php echo $title; ?></title>
    ...
    <link
	href="<?php echo config::$assets ?>files.photo.gallery@<?php echo config::$version ?>/css/files.css"
	rel="stylesheet">
...
function check_login($is_doc) {
	...
	if (! $is_logged_in) {
		...
		if ($is_doc) {
			...
			if ($is_login_attempt && mbstrtolower ( trim ( $_POST ['fusername'] ) ) == mbstrtolower ( config::$username ) && (phpversion () >= 5.5 && ! password_needs_rehash ( config::$password, PASSWORD_DEFAULT ) ? password_verify ( $fpassword, config::$password ) : ($fpassword == config::$password || md5 ( $fpassword ) == config::$password)) && $_POST ['client_hash'] === $client_hash && $_POST ['sidx'] === $sidx) {
...
function get_file($path, $resize = false, $clone = false) {
	...
	if ($resize == 'video') {
		...
		if ($cache)
			read_file ( $cache, null, 'Video thumb served from cache', null, true, $clone );
		...
		if (file_exists ( $cache ) && ! filesize ( $cache ) && imagejpeg ( imagecreate ( 1, 1 ), $cache ))
			read_file ( $cache, 'image/jpeg', '1px placeholder image created and cached', null, true, $clone );
...
function get_dir($path, $files = false, $json_url = false) {
	...
	$arr = array (
			// 'basename' => _basename($realpath) ?: _basename($path) ?: '',
			'basename' => $basename || $basename === '0' ? $basename : '','fileperms' => substr ( sprintf ( '%o', fileperms ( $realpath ) ), - 4 ),'filetype' => 'dir','is_readable' => $is_readable,'is_writeable' => is_writeable ( $realpath ),'is_link' => $symlinked ? is_link ( $path ) : false,'is_dir' => true,'mime' => 'directory','mtime' => $filemtime,'path' => root_relative ( $path ) );
...
function get_include($file) {
	if (! config::$storage_path)
		return;
	$query_string = strstr ( $file, '?' );
	$file_without_query = $query_string ? strstr ( $file, '?', true ) : $file;
	$path = config::$storage_path . '/' . $file_without_query;
	if (! file_exists ( $path ))
		return;
	...
	if (! config::$storage_is_within_doc_root)
		return;
	$src = get_url_path ( $path ) . ($query_string ?: '') . '?' . filemtime ( $path );
	if ($ext === 'js')
	...
	if ($action === 'fm') {
		...
		if ($task === 'upload') {
			...
			if (! empty ( $allowed_file_types )) {
				...
				if (function_exists ( 'exif_imagetype' ) && in_array ( $ext, [ '.gif','.jpeg','.jpg','.png','.swf','.psd','.bmp','.tif','.tiff','webp','avif' ] ) && ! @exif_imagetype ( $file ['tmp_name'] ))
			...
			if (@move_uploaded_file ( $file ['tmp_name'], "$path/$filename" ))
				fm_json_exit ( true, array ('success' => true,'filename' => $filename, // return filename in case it was incremented or renamed
		...
		json_exit ( array ('success' => $latest,'writeable' => $latest && is_writable ( __FILE__ ) // only check writeable if $latest
		) );
	...
	if (get ( 'download_dir_zip' )) {
		...
		if (! empty ( $files )) {
			...
			foreach ( $files as $file ) {
				...
				if (is_exclude ( $file, false ) || ! is_readable ( $file ))
					continue;
		...
		function get_watermark_files() {
			if (! config::$config ['allow_upload'] || ! config::$storage_path)
				return false;
		...
		function php_directive_value_to_bytes($directive) {
			...
			if (! isset ( $matches ['value'] ) || ! isset ( $matches ['option'] ))
				return 0;
...
<body class="body-loading">
	<main id="main">
		<nav id="topbar"
			<?php if(!empty(config::$config['topbar_sticky'])) echo ' class="topbar-sticky"'; ?>>
			<div id="topbar-top">
				<div id="search-container">
					<input id="search" class="input" type="search" placeholder="search"
						size="1" spellcheck="false" autocomplete="off" autocorrect="off"
		...
		<div id="files-container">
			<div id="files"
				class="list files-<?php echo config::$config['layout']; ?>"></div>
		</div>
	</main>
<?php if($menu_exists) { ?>
    <aside id="sidebar">
...
var CodeMirror = {};
    ...
    <?php
		...
		get_include ( 'js/functions.js' );

		// load _files/js/custom.js if exists
		if ($gadget == 'photo') {
			get_include ( "js/custom_photo.js?gadget=$gadget&id=$id" );
		} else
			get_include ( 'js/custom.js' );

		// load all Javascript assets
		foreach ( array_filter ( [ 'toastify-js@1.12.0/src/toastify.min.js','sweetalert2@11.7.20/dist/sweetalert2.min.js','animejs@3.2.1/lib/anime.min.js','@exeba/list.js@2.3.1/dist/list.min.js','yall-js@3.2.0/dist/yall.min.js','filesize@9.0.11/lib/filesize.min.js','screenfull@5.2.0/dist/screenfull.min.js','dayjs@1.11.9/dayjs.min.js','dayjs@1.11.9/plugin/localizedFormat.js','dayjs@1.11.9/plugin/relativeTime.js',(in_array ( config::$config ['download_dir'], [ 'zip','files' ] ) ? 'js-file-downloader@1.1.25/dist/js-file-downloader.min.js' : false),(config::$config ['download_dir'] === 'browser' ? 'jszip@3.10.1/dist/jszip.min.js' : false),(config::$config ['download_dir'] === 'browser' ? 'file-saver@2.0.5/dist/FileSaver.min.js' : false),'codemirror@5.65.14/mode/meta.js','files.photo.gallery@' . config::$version . '/js/files.js' ] ) as $key )
