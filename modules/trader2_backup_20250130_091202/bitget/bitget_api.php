...
class BitGetAPI
{
    ...
    private function request($method, $path, $params = [], $body = '')
    {
        ...
        // Für Spot-API: Parameter alphabetisch sortieren
        if (strpos($path, '/api/spot/') === 0) {
            ksort($params);
        }
    ...
    {
        $response = $this->request('GET', '/api/mix/v1/order/current', [
            'symbol' => $this->formatSymbol($symbol),
            'productType' => 'umcbl'  // USDT-M Perpetual
        ]);

        error_log("BitGet Active Orders Response: " . json_encode($response));

        if (!isset($response['data'])) {
            throw new Exception("Keine Order-Daten in der API-Antwort");
        }
        ...
        $orders = array_map(function($order) {
            return [
                ...
                'orderCategory' => $order['orderCategory'] ?? '',  // normal, tp (take profit), sl (stop loss)
    ...
    public function getPositions()
    {
        ...
        error_log("BitGet Positions Response: " . json_encode($response));
        ...
        $positions = array_map(function($position) {
            return [
                ...
                'averageOpenPrice' => $position['averageOpenPrice'] ?? '0',
    ...
    public function getAccountBills($startTime = null, $endTime = null, $limit = 100)
    {
