...
function get_th($arr, $position = false) {
	...
	if (is_array ( $arr ['checkbox'] ) && ! $position) {
		...
		$output .= "<th $checkbox_add_tb_class><div class='ui master checkbox'><input class='checkbox-main-{$arr['list']['id']}' value='{$arr['list']['id']}' type='checkbox' name='type'><label>" . $arr ['checkbox'] ['title'] . "</label></div></th>";
	/**
	 * **** END - Checkbox *********************************************
	 */
	}
