...
if (isset ( $_GET ['operation'] )) {
	$fs = new tree ( db::get ( 'mysqli://root@127.0.0.1/test' ), array (
	...
	try {
		...
		switch ($_GET ['operation']) {
			...
			case 'create_node' :
				$node = isset ( $_GET ['id'] ) && $_GET ['id'] !== '#' ? ( int ) $_GET ['id'] : 0;
				$temp = $fs->mk ( $node, isset ( $_GET ['position'] ) ? ( int ) $_GET ['position'] : 0, array (
						'nm' => isset ( $_GET ['text'] ) ? $_GET ['text'] : 'New node' 
				) );
				$rslt = array (
						'id' => $temp 
				);
			...
			case 'rename_node' :
				...
				$rslt = $fs->rn ( $node, array (
						'nm' => isset ( $_GET ['text'] ) ? $_GET ['text'] : 'Renamed node' 
				) );
				break;
			case 'delete_node' :
				$node = isset ( $_GET ['id'] ) && $_GET ['id'] !== '#' ? ( int ) $_GET ['id'] : 0;
...
<body>
	<div id="container" role="main">
		...
		<div id="data">
			...
			<div class="content default" style="text-align: center;">Select a
				node from the tree.</div>
		</div>
	</div>
