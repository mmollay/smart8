...
namespace
{
	...
	{
		if ($c === null) { $c = \vakata\cache\
			cache::inst ();
		}
			return new \vakata\database\
		DBCCached ( $settings, $c );
	}
}
}
...
namespace vakata\database
{
	...
	class mysql_driver extends ADriver
	{
		...
		public function connect() {
			...
			if($this->lnk === false || !mysql_select_db($this->settings->database, $this->lnk) || !$GLOBALS['mysqli']->query("SET NAMES '".$this->settings->charset."'", $this->lnk)) {
				throw new Exception('Connect error: ' . mysqli_error());
			}
			if($this->settings->timezone) {
				@$GLOBALS['mysqli']->query("SET time_zone = '" . addslashes($this->settings->timezone) . "'", $this->lnk);
			}
			return true;
		}
		public function disconnect() {
		...
		public function real_query($sql) {
			if(!$this->is_connected()) { $this->connect(); }
			$temp = $GLOBALS['mysqli']->query($sql, $this->lnk);
			if(!$temp) {
				throw new Exception('Could not execute query : ' . mysqli_error($this->lnk) . ' <'.$sql.'>');
			}
			$this->iid = $GLOBALS['mysqli']->insert_id($this->lnk);
		...
		public function seek($result, $row) {
			...
			if(!$temp) {
				//throw new Exception('Could not seek : ' . mysqli_error($this->lnk));
			}
			return $temp;
		}
		public function nf($result) {
		...
		protected function escape($input) {
			...
			if(is_string($input)) {
				$input = $GLOBALS['mysqli']->real_escape_string($input, $this->lnk);
				return "'".$input."'";
			}
			if(is_bool($input)) {
				return $input === false ? 0 : 1;
