...
// Entferne die getDatabase-Funktion, wenn sie innerhalb von getJobStats definiert ist
$pattern = "/function getJobStats.*?{(.*?)function getDatabase\(\) \{.*?return \\\$db;\s*\}(.*?)}/s";
$replacement = "function getJobStats\$1\$2}";
$content = preg_replace($pattern, $replacement, $content);

// Speichere die Datei
if (file_put_contents($file, $content)) {
    echo "newsletters.php wurde korrigiert. Die falsch platzierte getDatabase-Funktion wurde entfernt.\n";
} else {
    echo "Fehler beim Aktualisieren von newsletters.php\n";
}

// Erstelle ein neues Skript für die korrekte Implementierung
$fixScript = __DIR__ . '/fix_status_display_v3.php';
$fixContent = '<?php
// Lese die Datei ein
$file = __DIR__ . "/../lists/newsletters.php";
$content = file_get_contents($file);
...
file_put_contents($fixScript, $fixContent);
echo "Fix-Skript für Status-Anzeige wurde erstellt: $fixScript\n";
echo "Bitte führe dieses Skript nach der Behebung des Function-Issues aus.\n";
?>
