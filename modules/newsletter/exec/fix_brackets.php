...
// Ursprünglicher Inhalt für einen Neustart
$originalFile = $file . '.original';
if (!file_exists($originalFile)) {
    file_put_contents($originalFile, $content);
    echo "Original von process_batch.php gesichert: $originalFile\n";
}

// Statt zu versuchen, einzelne Klammern zu finden, erstellen wir eine neue Version der Datei
// mit der korrekten Struktur
...
function processJob($db, $emailService, $placeholderService, $contentId, $jobId, $newsletter = null) {
    try {
        ...
        $subject = $placeholderService->replacePlaceholders($newsletter['subject'], $recipient, $customFields);
        $content = $placeholderService->replacePlaceholders($newsletter['content'], $recipient, $customFields);
        ...
        // E-Mail senden
        $messageId = $emailService->sendSingleEmail(
            $contentId,
            $sender,
            $recipient,
            $subject,
        ...
        $stmt = $db->prepare("
            UPDATE email_jobs 
            SET status = ?, message_id = ?, processed_at = NOW() 
            WHERE id = ?
        ");
        $status = 'sent';
        ...
        return [
            'success' => true,
            'message' => "E-Mail gesendet an {$recipient['email']} mit Message-ID: $messageId"
        ];
    } catch (Exception $e) {
...
if (file_put_contents($file, $newContent)) {
    echo "process_batch.php wurde neu geschrieben mit korrekter Klammerstruktur.\n";
} else {
