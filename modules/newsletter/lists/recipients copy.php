...
$listGenerator->addExternalButton('export', [
    ...
    'alignment' => 'right',
    'title' => 'Export',
    'popup' => ['content' => 'Empfängerliste exportieren'],
...
$listGenerator->addScript("
$(document).ready(function() {
    window.exportRecipients = function() {
        let filters = {};
        $('#" . $listGenerator->getConfig()['contentId'] . " .ui.dropdown[id^=\"filter_\"]').each(function() {
            let filterId = $(this).attr('id');
            let value = $(this).dropdown('get value');
            if (value) {
                let filterName = filterId.replace('filter_" . $listGenerator->getConfig()['contentId'] . "_', '');
                filters[filterName] = value;
            }
        });

        let search = $('#search_" . $listGenerator->getConfig()['contentId'] . "').val();
        
        let url = 'ajax/export_recipients.php';
        let params = new URLSearchParams({
            listId: '" . $listGenerator->getConfig()['listId'] . "',
            filters: JSON.stringify(filters),
            search: search,
            sort: '" . $listGenerator->getConfig()['sortColumn'] . "',
            sortDir: '" . $listGenerator->getConfig()['sortDirection'] . "'
        });
