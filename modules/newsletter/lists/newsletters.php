...
$columns = [
    ...
    [
        ...
        'formatter' => function ($value, $row) {
            ...
            // Status-Details sammeln
            $details = [];

            if ($blacklisted > 0) {
                $details[] = "<span class='ui black text'>{$blacklisted} auf Blacklist</span>";
            }
            if ($failed > 0) {
            ...
            // Wenn alles versendet wurde
            if (($sent + $failed + $blacklisted) >= $total) {
                return "
                <div>
                    <span class='ui green text'><i class='check circle icon'></i> Vollständig verarbeitet</span>
                    {$detailsHtml}
                </div>";
            }

            // Wenn teilweise versendet
