...
require_once (__DIR__ . '/class.multicelltag.php');
...
class FPDF_TABLE extends FPDF_MULTICELLTAG
{
	...
	var $tb_split_normal; /*
	   * << ** special request from Marc Ulfig >>
	 ...
	 * 
	 * @param $data -
	 *        	array containing line informations
	...
	function DrawTableLine($data, $header, $pDataType = 0)
	{
		...
		for ($i = 0; $i < $this->tb_columns; $i++) {
			...
			if (!isset($data[$i]['TEXT_STRLINES']))
				$data[$i]['TEXT_STRLINES'] = $this->StringToLines($data[$i]['CELL_WIDTH'], $data[$i]['TEXT']);
		...
		for ($i = 0; $i < $this->tb_columns; $i++) {
			...
			// print the text
			$this->MultiCellTable($data[$i]['CELL_WIDTH'], $data[$i]['LN_SIZE'], $data[$i]['TEXT_STRLINES'], $data[$i]['BRD_TYPE'], $data[$i]['T_ALIGN'], $data[$i]['V_ALIGN'], 1, $h - $data[$i]['LN_SIZE'] * $data[$i]['CELL_LINES']);
	 ...
	 * 
	 * @param $header -
	 *        	boolean - if the header is drawed or not
	 * @return nothing
	 */
...
// Klasse wird zusaetzlich durch die Erweiterung eines Headers und Footers erweitert
class FPDF_MARTIN extends FPDF_TABLE
{
	// Kopfzeile
	function Header()
	{
		// Zeilenabstand von oben
		$this->SetY(29);
		// Logo

		if (is_file($_SESSION['faktura_header'])) {
			$this->Image($_SESSION['faktura_header'], 10, 8, 190);
			// $this->PDF_ImageAlpha($_SESSION['faktura_header'],10,8,190, 0);
		} else {
			// if ($_SESSION['compangy_id'])
			// $this->Image("../ssi_faktura/images/logo{$_SESSION['compangy_id']}.jpg",10,8,190);
		}
		// Linie
		// $this->Cell(0,0,'','T',1,'C');
	...
	function Footer()
	{
		...
		// Seitenzahl
		// $this->Cell(0,7,'Seite '.$this->PageNo().'/{nb}',0,1,'C');

		// Arial kursiv 8
		$this->SetFont('Arial', 'I', 8);

		// Wenn Content aus dem Textbaustein kommt
		if ($content_footer) {
			$this->MultiCell(0, 4, $content_footer, 'T', 1, 'C');
		} else {
			$this->Cell(0, 4, $fusszeile['text1'] . $fusszeile['text2'], 'T', 1, 'C');
			if ($fusszeile['text4'])
				$this->Cell(0, 4, $fusszeile['text4'], 0, 1, 'C');
			if ($fusszeile['text3'])
				$this->Cell(0, 4, $fusszeile['text3'], 0, 1, 'C');
		}
	}
}
